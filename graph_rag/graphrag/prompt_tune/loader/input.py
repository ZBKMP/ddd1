...
async def load_docs_in_chunks(
    ...
    if len(chunks_df) == 0:
        msg = (
            "No text chunks available for prompt tuning. Ensure input documents "
            "contain readable text (for PDF projects, run indexing first or place "
            "markdown/text files under pdf_output/ or prompt_tune_input/)."
        )
        raise ValueError(msg)

    effective_limit = min(limit, len(chunks_df))

    if select_method == DocSelectionType.TOP:
        chunks_df = chunks_df[:effective_limit]
    elif select_method == DocSelectionType.RANDOM:
        chunks_df = chunks_df.sample(n=effective_limit)
