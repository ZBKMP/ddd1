...
def _parse_json_model_response(
    content: str | None,
    json_model: type[BaseModel] | None = None,
) -> BaseModel | dict[str, Any] | None:
    """Parse LLM JSON content; repair markdown-wrapped or partial JSON when needed."""
    ...
    if not text:
        ...
        try:
            ...
            _, parsed_dict = try_parse_json_object(text, verbose=True)

    if json_model is not None and inspect.isclass(json_model) and issubclass(
        json_model, BaseModel
    ):
        return json_model(**parsed_dict)
...
class LitellmChatModel:
    ...
    def _get_kwargs(self, **kwargs: Any) -> dict[str, Any]:
        ...
        # Use generic json_object for the API; validate with json_model after the response.
        # Many OpenAI-compatible proxies do not support Pydantic response_format schemas.
        if kwargs.get("json"):
    ...
    async def achat(
        ...
        parsed_response: BaseModel | dict[str, Any] | None = None
        if "response_format" in new_kwargs:
            ...
            parsed_response = _parse_json_model_response(
