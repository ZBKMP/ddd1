...
import { ref } from 'vue'
import { useRoute } from 'vue-router' //导入路由类
import { Message } from '@arco-design/web-vue'
import { debugApp } from '@/services/app' //导入debugApp方法

//定义交互所需要的数据
const query = ref('')
const messages = ref<any[]>([])
const isLoading = ref(false)
const route = useRoute() //创建路由对象 获取路由信息
...
const send = async () => {
  ...
  try {
    ...
    // 发起请求后 清空输入框  query.value实现了与页面的双向绑定,输入框会被清空
    query.value = ''
    // 更改状态为正在加载状态
    isLoading.value = true

    // 调用异步的debugApp函数 发起http请求
    const response = await debugApp(route.params.app_id as string, humanQuery)
    // 接收响应结果 类型为DebugAppResponse , 从中提取出 data 再提取出content
    const content = response.data.content

    //将响应成功的ai消息加入到消息列表中 页面会显示所有历史消息
    messages.value.push({
      role: 'ai',
      content: content,
    })
          ...
          <div class="flex flex-row gap-2 mb-6" v-for="message in messages" :key="message.content">
            ...
            <div class="flex flex-col gap-2">
              <div class="font-semibold text-gray-700">
                {{ message.role === 'human' ? '小黑子' : 'ChatGPT聊天机器人' }}
              </div>
              <div
                v-if="message.role === 'human'"
