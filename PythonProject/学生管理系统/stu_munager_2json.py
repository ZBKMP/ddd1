...
#方法 加载json文件内容到stu_list文件中
#在加载空json文件时 先填入"[]"避免空内容而抛异常
def load_data_from_json_file():
    global stu_list #声明操作全局变量 sru_list
    with open(file="data.json",mode="r",encoding="utf-8") as f:
        stu_list = json.load(f)
#方法 将当前stu_list的内容 整体更新到json文件内
...
# 执行程序
load_data_from_json_file() #启动时加载文件数据到stu_list
while True:
    show_menu()
