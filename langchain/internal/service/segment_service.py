...
from internal.schema import GetSegmentsWithPageReq, CreateSegmentReq, UpdateSegmentReq
...
class SegmentService(BaseService):
    ...
    def get_segments_with_page(
        ...
        if req.search_word.data:
            # 传递了search_word参数 则按片段内容模糊查询
            filters.append(
                Segment.content.ilike(f"%{req.search_word.data}%")
            )
    ...
    def update_segment_enabled(
        ...
        # 2 判断文档片段是否处于可启用/禁用的环境 片段状态必须是以完成
        ...
        if enabled == segment.enabled:
            raise FailException(
                f"片段状态修改错误，当前已是{'启用' if enabled else '禁用'}状态"
            )

        # 4.获取更新片段启用状态锁并上锁检测
        cacha_key = LOCK_SEGMENT_UPDATE_ENABLED.format(segment_id=segment_id)
        cache_result = self.redis_client.get(cacha_key)
        if cache_result is not None:
            raise FailException("当前文档片段正在修改状态，请稍后尝试")

        # 5.上锁并更新对应的数据，涵盖postgres记录、weaviate、关键词表
        #  虽不需要进行异步操作,但包含步骤比较多,需要使用redis锁,防止在修改过程中被其他事务访问
        with self.redis_client.lock(cacha_key, LOCK_EXPIRE_TIME):
            try:
                # 6.修改数据库里的文档片段状态
                self.update(
                    segment,
                ...
                # Segment.document 只读属性 获取关联的Document实体对象
                ...
                logging.exception(
                    f"更改文档片段启用状态出现异常, "
                    f"segment_id: {segment_id}, 错误信息: {str(e)}"
                )

                # 将出错的片段数据改为不可用 以及状态为ERROR 记录错误信息
                self.update(
                    segment,
                    error=str(e),
    ...
    def delete_segment(
        ...
        if segment.status not in [
            SegmentStatus.COMPLETED, SegmentStatus.ERROR
        ]:
        ...
        # 5.同步删除向量数据库存储的记录
        try:
            self.vector_store_service.collection.data.delete_by_id(
        ...
        # 6.更新文档信息，涵盖字符总数、token总数
        # 重新统计查询文档的字符总数,token总数
    ...
    def create_segment(
        ...
        if token_count > 1000:
            raise ValidationException("片段内容的长度不能超过1000 token")
        ...
        try:
            ...
            self.vector_store_service.vector_store.add_documents(
                ...
                # 存储向量数据库时,自行生成ids数据,与documents结构保持一致
                ids=[str(segment.node_id)],
            )

            # 9.重新计算该Document下所有片段的字符总数以及token总数
            document_character_count, document_token_count = self.db.session.query(
                func.coalesce(func.sum(Segment.character_count), 0),
                func.coalesce(func.sum(Segment.token_count), 0)
            ).filter(Segment.document_id == document.id).first()

            # 10.更新Document文档的对应信息
            self.update(
                document,
                character_count=document_character_count,
                token_count=document_token_count,
    ...
    def update_segment(
        ...
        try:
            ...
            if required_update:
                ...
                # 9.更新向量数据库对应记录
                self.vector_store_service.collection.data.update(
                    uuid= str(segment.node_id), # ID为条件
                    # 修改文本内容 text
                    properties={
                        "text": req.content.data,
                    },
