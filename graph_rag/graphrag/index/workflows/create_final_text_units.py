...
_LIST_ID_COLUMNS = ("entity_ids", "relationship_ids", "covariate_ids")


def _unique_ids_as_list(ids: pd.Series) -> list:
    """Return unique ids as a plain Python list (parquet-safe)."""
    return ids.unique().tolist()


def _ensure_python_list(value) -> list:
    """Coerce numpy/Arrow array aggregates to plain lists for parquet export."""
...
def create_final_text_units(
    ...
    for col in _LIST_ID_COLUMNS:
        if col in aggregated.columns:
            aggregated[col] = aggregated[col].apply(_ensure_python_list)
...
def _entities(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        ...
        .agg(entity_ids=("id", _unique_ids_as_list))
        .reset_index()
...
def _relationships(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        unrolled.groupby("text_unit_ids", sort=False)
        .agg(relationship_ids=("id", _unique_ids_as_list))
        .reset_index()
        .rename(columns={"text_unit_ids": "id"})
    )
...
def _covariates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        ...
        .agg(covariate_ids=("id", _unique_ids_as_list))
        .reset_index()
