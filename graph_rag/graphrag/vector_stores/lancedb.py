...
def _resolve_lancedb_table_name(index_name: str, table_names: list[str]) -> str | None:
    """Match configured index_name to an existing LanceDB table."""
    if index_name in table_names:
        return index_name
    candidates: list[str] = []
    if index_name.startswith("default-"):
        candidates.append(index_name.removeprefix("default-"))
    if "-" in index_name:
        candidates.append(index_name.split("-", 1)[-1])
...
class LanceDBVectorStore(BaseVectorStore):
    ...
    def connect(self, **kwargs: Any) -> Any:
        ...
        if self.index_name:
            ...
            resolved_table = _resolve_lancedb_table_name(self.index_name, table_names)
            # #region agent log
            import json
            import time
            from pathlib import Path

            _log_path = Path(__file__).resolve().parents[2] / "debug-d938bd.log"
                    ...
                    json.dumps(
                        {
                            "sessionId": "d938bd",
                            "hypothesisId": "G",
                            "location": "lancedb.py:connect",
                            "message": "resolve lancedb table",
                            "data": {
    ...
    def similarity_search_by_vector(
        ...
        for doc in docs:
            raw_attributes = doc.get(self.attributes_field)
            attributes = (
