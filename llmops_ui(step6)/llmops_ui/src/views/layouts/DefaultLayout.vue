...
import AvatarUploadModal from '@/views/layouts/components/AvatarUploadModal.vue'

// 1.定义页面所需数据
const settingModalVisible = ref(false)
const avatarUploadModalVisible = ref(false)
...
<template>
  <a-layout has-sider class="h-full">
      ...
      <div class="bg-white h-full rounded-lg px-2 py-4 flex flex-col justify-between">
        ...
        <a-dropdown position="tl">
          ...
          >
            ...
            <a-avatar
              :size="32"
              class="text-sm bg-blue-700 shrink-0"
          ...
          <template #content>
            <a-doption @click="avatarUploadModalVisible = true">
              <template #icon>
                <icon-camera />
              </template>
              更换头像
    ...
    <!-- 更换头像 -->
    <avatar-upload-modal v-model:visible="avatarUploadModalVisible" />
  </a-layout>
</template>
