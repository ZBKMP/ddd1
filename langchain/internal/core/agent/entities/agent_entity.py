...
class AgentConfig(BaseModel):
    """智能体配置信息,包含 : LLM大语言模型、预设prompt、关联插件、知识库、工作流、是否开启长期记忆等内容，
                         后期可以随时扩展"""
    # 智能体使用的LLM
    ...
    # 预设提示,默认为空,该值由用户在编排的时候记录,并填充到system_prompt中
    preset_prompt: str = ""

    # 智能体长期记忆是否开启
    enable_long_term_memory: bool = False  # 是否开启会话长期记忆(信息摘要汇总)

    # 智能体使用的工具列表
    tools: list[BaseTool] = Field(default_factory=list)


# 自定义图状态数据 继承自 MessagesState(包含messages及归纳函数)
