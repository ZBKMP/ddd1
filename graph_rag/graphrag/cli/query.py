...
from graphrag.utils.cli import resolve_query_output_dir
...
_GLOBAL_SEARCH_TABLES = ("entities", "communities", "community_reports")
_BASIC_SEARCH_TABLES = ("text_units",)


def _load_config_for_query(
    root: Path,
    config_filepath: Path | None,
    data_dir: Path | None,
    required_tables: tuple[str, ...],
) -> GraphRagConfig:
    """Load config and resolve a complete index output directory for query."""
    cli_overrides: dict[str, str] = {}
    if data_dir:
        cli_overrides["output.base_dir"] = str(data_dir)
    config = load_config(root, config_filepath, cli_overrides or None)
    if data_dir is None:
        configured_output = Path(config.output.base_dir)
        resolved_output = resolve_query_output_dir(
            project_root=root,
            configured_output=configured_output,
            required_tables=required_tables,
        )
        if resolved_output.resolve() != configured_output.resolve():
            print(
                f"Warning: index not found in {configured_output}; "
                f"using parent project config and index at {resolved_output}"
            )
            # Parent index was built with parent settings (vector store table names, etc.)
            return _load_config_for_query(
                root.parent,
                config_filepath,
                resolved_output,
                required_tables,
...
def run_global_search(
    ...
    config = _load_config_for_query(
        root, config_filepath, data_dir, _GLOBAL_SEARCH_TABLES
    )
...
def run_local_search(
    ...
    root = root_dir.resolve()
    config = _load_config_for_query(
        root, config_filepath, data_dir, _LOCAL_SEARCH_TABLES
    )
...
def run_basic_search(
    ...
    root = root_dir.resolve()
    config = _load_config_for_query(
        root, config_filepath, data_dir, _BASIC_SEARCH_TABLES
    )
